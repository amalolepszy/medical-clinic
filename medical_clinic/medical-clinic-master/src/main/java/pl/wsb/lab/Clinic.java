...
public class Clinic {
    ...
    public Clinic(DoctorRegistry doctorRegistry,  DoctorShiftRegistry doctorShiftRegistry) {
        this.doctorRegistry = doctorRegistry;
        this.doctorShiftRegistry = doctorShiftRegistry;
    }
    ...
    //4.1
    public void createDoctorShift(Doctor doctor, LocalTime startShift, LocalTime endShift, LocalDate shiftDay) {
        if (doctor == null || startShift == null || endShift == null || shiftDay == null) {
        ...
        if (!doctorRegistry.getDoctors().contains(doctor)) {
            throw new IllegalArgumentException("This doctor does not exist" + doctor);
        }
        ...
        for(DoctorShift doctorShift: doctorShifts){
            if(doctorShift.getDoctor().equals(doctor) && doctorShift.getShiftDay().isEqual(shiftDay)) {
